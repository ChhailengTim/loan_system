            ...
            <b-row>
                ...
                <b-col cols="6" md="3" xl="2">
                    ...
                    <b-col cols="12">
                        ...
                        <div v-if="$helpers.nullToVoid(form.family_book) == ''">
                            <i
                                class="fas fa-image"
                                style="font-size: 150px; color: #a0aec0"
                            ></i>
                    ...
                    <b-col cols="12" class="mt-2">
                        <b-button
                            ...
                            style="width: 145px;"
                            variant="outline-primary"
                            @click="chooseFamilyBook"
                        >{{ $t('choose_file') }}</b-button>
                        <input
                            ref="fileInputFamilyBook"
                            style="display: none"
                            type="file"
                            accept=".png, .jpg"
                            :multiple="false"
                            @change="onFileChangeFamilyBook"
                        />
                    </b-col>
                </b-col>
...
export default {
    ...
    methods: {
        ...
        chooseFamilyBook() {
                this.$refs.fileInputFamilyBook.click()
        },
        onFileChangeFamilyBook($event) {
            let vm = this
            const files = $event.target.files || $event.dataTransfer.files
            if (files[0]) {
                if (files[0].size > 2 * 1024 * 1024) {
                ...
                reader.onload = function (e) {
                    vm.form.family_book = '';
                    vm.form.family_book = e.target.result
                }
                reader.readAsDataURL(files[0])
            } else {
                vm.form.family_book = ''
